...
export default function About() {
  ...
  return (
    <section className="text-gray-600 body-font py-12">
      ...
      <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 px-6 md:px-[121px] py-8">
        {[
          ...
          <div key={index} className="bg-[#F9F9F9] px-[28px] py-[30px] md:px-[28px] py-[30px] rounded-lg shadow-md text-teal-600 flex flex-col justify-between">
            <span className="block mb-4 text-lg ">{item.emoji}</span>
            <h2 className="text-2xl md:text-[20px]  mb-4">{item.title}</h2>
            <p className="text-teal-600 text-[16px] md:text-lg">{item.text}</p>
          </div>
        ))}
      </div>

      {/* Featured Products Section */}
      <div className="px-4 sm:px-8 md:px-[5px] py-8">
